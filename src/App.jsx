 ...
 // --- STATE UNTUK CATATAN APLIKASI ---
  ...
  const checkTimeOverlap = (activitiesToCheck, existingData) => {
        ...
        for (let j = i + 1; j < entries.length; j++) {
          ...
          for (const s1 of a.segs) {
            for (const s2 of b.segs) {
              ...
              if (y2 <= x2) y2 += 1440;
              if (x1 < y2 && x2 < y1) {
                oStart = Math.max(x1, x2);
                oEnd = Math.min(y1, y2);
                break;
              }
            }
            if (oStart !== null) break;
          }
          if (oStart !== null) overlaps.push({ date, a, b, oStart, oEnd });
        }
      }
    });
    ...
    const describe = (member) => {
      ...
      member.segs.forEach(s => {
        if (!s.start || !s.end) return;
        let a = toMins(s.start), e = toMins(s.end);
        if (e <= a) e += 1440;
        startMins = Math.min(startMins, a);
        endMins = Math.max(endMins, e);
        hasSeg = true;
      });
      const startTxt = hasSeg ? fmtMins(startMins) : '?';
  ...
  const handleParse = async () => {
    ...
    // --- HELPER BARU: PECAH AKTIVITAS LINTAS TENGAH MALAM PER HARI ---
    // Contoh: Tidur 10/9 23.05 - 11/9 05.49 dipecah menjadi:
    //   10/9 23.05 - 23.59  dan  11/9 00.01 - 05.49
    // Batas antar hari: hari pertama berakhir 23.59, hari berikutnya mulai 00.01.
    const splitOvernightActivity = (act) => {
      if (!act.endDate || act.endDate === act.date) return [act];

      const srcLines = Array.isArray(act._srcLines) ? [...act._srcLines] : [];
      const baseCreated = typeof act.createdAt === 'number' ? act.createdAt : Date.now();
      const makePart = (date, start, end, offset) => {
        const part = {
          ...act,
          ...
          endTime: end,
          segments: [{ start, end }],
          createdAt: baseCreated + offset,
        };
        if (srcLines.length > 0) part._srcLines = srcLines;
        return finalizeSession(part);
      };

      const parts = [];
      let curDate = act.date;
      let guard = 0;

      // Bagian hari pertama + hari tengah (jika ada), sampai sebelum hari terakhir
      while (curDate !== act.endDate && guard < 365) {
        guard++;
        const pStart = curDate === act.date ? act.startTime : '00.01';
        const pEnd = '23.59';
        if (toMinOfDay(pEnd) > toMinOfDay(pStart)) {
          parts.push(makePart(curDate, pStart, pEnd, parts.length));
        }
        curDate = addDays(curDate);
      }

      // Bagian hari terakhir (act.endDate); segmen kosong (selesai 00.00) dihilangkan
      if (toMinOfDay(act.endTime) > toMinOfDay('00.01')) {
        parts.push(makePart(act.endDate, '00.01', act.endTime, parts.length));
      }
    ...
    const preMergeActivities = [];
    newActivities.forEach(act => {
      const parts = splitOvernightActivity(act);
      if (parts.length > 1) {
        const lineNums = Array.isArray(act._srcLines) ? act._srcLines : [];
        const partDesc = parts.map(p => `${p.date} ${p.startTime}-${p.endTime}`).join(', ');
        lineNums.forEach(n => {
          if (traceLines[n - 1] && !traceLines[n - 1].notes.some(t => t.startsWith('Sesi melewati tengah malam'))) {
            traceLines[n - 1].notes.push(`Sesi melewati tengah malam => dipecah per hari: ${partDesc}`);
          }
        });
      }
      preMergeActivities.push(...parts);
    });
    ...
    const groupedActs = {};
    preMergeActivities.forEach(act => {
    ...
    // Data hasil parsing ke depannya wajib diverifikasi ulang lewat wizard
  ...
  // Hanya aktivitas yang ditandai isInputVerified: false (hasil parsing baru)
  // yang masuk antrian; data lama tanpa field dianggap sudah terverifikasi.
...
{/* Semua konten dalam satu area scroll: identitas aktivitas, cuplikan, tombol */}
        ...
        {verifDeleteTarget && (
          <div className="fixed inset-0 bg-gray-900/60 z-[70] flex items-center justify-center p-6 backdrop-blur-sm">
            <div className={`w-full max-w-sm rounded-[32px] p-6 shadow-2xl animate-in zoom-in-95 duration-200 ${isDarkMode ? 'bg-gray-900 border border-gray-800' : 'bg-white border border-gray-100'}`}>
              <div className="w-14 h-14 mx-auto rounded-2xl bg-red-500/10 flex items-center justify-center mb-3">
                <svg className="w-7 h-7 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
              </div>
              <h3 className={`text-lg font-extrabold mb-2 text-center ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>Hapus Aktivitas?</h3>
        ...
        {verifCompletionPopup && (
          <div className="fixed inset-0 bg-gray-900/60 z-[80] flex items-center justify-center p-6 backdrop-blur-sm">
            <div className={`w-full max-w-sm rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95 duration-200 text-center ${isDarkMode ? 'bg-gray-900 border border-gray-800' : 'bg-white border border-gray-100'}`}>
              <div className="w-20 h-20 mx-auto mb-4 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center shadow-xl shadow-emerald-500/30">
